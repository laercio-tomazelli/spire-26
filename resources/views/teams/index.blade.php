...
<x-layouts.module title="Times">
    ...
    @endphp
    <div id="teams-table-container" data-url="{{ route('teams.index') }}" data-csrf="{{ csrf_token() }}"
        ...
        <x-ui.table>
            ...
            <x-slot:header>
                ...
                <x-ui.table.tabs>
                    <x-ui.table.tab :active="!request('status')" :count="$counts['all'] ?? $teams->total()"
                ...
                {{-- Toolbar --}}
                <x-ui.table.header :search="true" searchPlaceholder="Buscar times...">
                    ...
                    <x-slot:columnManager>
                        <x-ui.table.column-manager>
                            <x-ui.table.column-toggle name="team" label="Time" :checked="true"
                                :disabled="true" />
            ...
            <div class="fi-ta-content">
                @include('teams.partials.table-filament', ['teams' => $teams])
            </div>
        </x-ui.table>
    </div>
