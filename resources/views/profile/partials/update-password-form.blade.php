    ...
    <form method="post" action="{{ route('password.update') }}" class="mt-6 space-y-6">
        ...
        <div class="flex items-center gap-4">
            ...
            @if (session('status') === 'password-updated')
                <p id="password-saved-message"
                    class="text-sm text-gray-600 dark:text-gray-400 transition-opacity duration-300">{{ __('Salvo.') }}
                </p>
                <script>
                    setTimeout(() => {
                        const el = document.getElementById('password-saved-message');
