...
<th
    ...
    @if ($sortable)
        <button type="button" class="group inline-flex items-center gap-x-1 whitespace-nowrap"
            ...
            <span
                class="flex-none rounded text-gray-400 group-hover:text-gray-500 dark:text-gray-500 dark:group-hover:text-gray-400">
                {{-- Ascending icon --}}
                <svg x-show="sortField === '{{ $sortField ?? Str::snake($label) }}' && sortDirection === 'asc'"
                ...
                {{-- Descending icon --}}
                <svg x-show="sortField === '{{ $sortField ?? Str::snake($label) }}' && sortDirection === 'desc'"
                    class="h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                    <path fill-rule="evenodd"
                ...
                <svg x-show="sortField !== '{{ $sortField ?? Str::snake($label) }}'"
