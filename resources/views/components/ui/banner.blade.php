...
<div id="{{ $bannerId }}" role="alert"
    {{ $attributes->merge(['class' => "{$positionClass} {$typeClass} transition-all duration-300"]) }}>
        ...
        <div class="flex items-center justify-between gap-4 py-3 {{ $center ? 'sm:justify-center' : '' }}">
            ...
            @if ($dismissible)
                <button type="button"
                    ...
                    onclick="dismissBanner('{{ $bannerId }}', {{ $persist ? 'true' : 'false' }})"
                    aria-label="Fechar">
...
<script>
    (function() {
        ...
        const persist = {{ $persist ? 'true' : 'false' }};
        const fixed = {{ $fixed ? 'true' : 'false' }};

        // Check if should be hidden from localStorage
        if (persist && localStorage.getItem('banner-' + bannerId + '-dismissed')) {
        ...
        window.dismissBanner = window.dismissBanner || function(id, shouldPersist) {
            ...
            if (shouldPersist) {
                localStorage.setItem('banner-' + id + '-dismissed', 'true');
            }
        };
    })();
</script>
