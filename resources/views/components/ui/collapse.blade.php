...
<div data-collapse="{{ $collapseId }}" {{ $attributes->merge(['class' => $variantClass]) }}>
    {{-- Header/Trigger --}}
    <button type="button"
        ...
        @if (!$disabled) onclick="SpireUI.collapse('{{ $collapseId }}')?.toggle()" @endif
        aria-expanded="{{ $open ? 'true' : 'false' }}" aria-controls="{{ $collapseId }}-content">
        @if ($icon && $iconPosition === 'left')
            <svg class="w-5 h-5 text-gray-500 dark:text-gray-400 transition-transform duration-200 shrink-0"
                data-collapse-icon style="{{ $open ? 'transform: rotate(180deg)' : '' }}" fill="none"
    ...
    {{-- Content --}}
    <div id="{{ $collapseId }}-content" class="overflow-hidden transition-all duration-300 ease-in-out"
        style="{{ $open ? '' : 'max-height: 0; opacity: 0;' }}" data-collapse-content>
