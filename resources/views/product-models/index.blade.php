...
<x-layouts.module title="Modelos de Produto">
    {{-- Breadcrumbs --}}
    <x-slot:breadcrumbs>
        <x-spire::breadcrumbs :items="[['label' => 'Dashboard', 'href' => route('dashboard')], ['label' => 'Modelos de Produto']]" />
    </x-slot:breadcrumbs>

    {{-- Header --}}
    <x-slot:header>
        Gerencie os modelos de produto do sistema
    </x-slot:header>
    ...
    @endphp
    <div id="product-models-table-container" data-url="{{ route('product-models.index') }}"
        ...
        <x-ui.table>
            ...
            <x-slot:header>
                ...
                <x-ui.table.tabs>
                    <x-ui.table.tab :active="!request('status')" :count="$counts['all'] ?? $productModels->total()"
                ...
                {{-- Toolbar --}}
                <x-ui.table.header :search="true" searchPlaceholder="Buscar modelos...">
                    ...
                    <x-slot:filters>
                        <x-ui.table.filters :activeCount="$activeFiltersCount ?? 0">
                            <div>
                                ...
                                <x-spire::select name="filter_brand_id" placeholder="Todas" :options="$brands
                            ...
                            <div>
                                ...
                                <x-spire::select name="filter_product_category_id" placeholder="Todas"
                                    :options="$categories
                                        ->map(fn($c) => ['value' => (string) $c->id, 'label' => $c->name])
                                        ->values()
                                        ->toArray()" :value="request('product_category_id', '')" />
                            </div>
                    ...
                    <x-slot:columnManager>
                        <x-ui.table.column-manager>
                            <x-ui.table.column-toggle name="product" label="Modelo" :checked="true"
                                :disabled="true" />
                            <x-ui.table.column-toggle name="brand" label="Marca" :checked="true" />
                            <x-ui.table.column-toggle name="category" label="Categoria" :checked="true" />
                            <x-ui.table.column-toggle name="warranty" label="Garantia" :checked="true" />
            ...
            <div class="fi-ta-content">
                @include('product-models.partials.table-filament', ['productModels' => $productModels])
            </div>
        </x-ui.table>
    </div>
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', () => {
                const container = document.getElementById('product-models-table-container');
                if (!container) return;
                ...
                window.addEventListener('select-change', (e) => {
                    ...
                    const keyMap = {
                        ...
                        'filter_product_category_id': 'product_category_id'
                    };
