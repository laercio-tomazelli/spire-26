<x-layouts.module title="Editar Peça">
    {{-- Breadcrumbs --}}
    <x-slot:breadcrumbs>
        <x-spire::breadcrumbs :items="[
            ['label' => 'Dashboard', 'href' => route('dashboard')],
            ['label' => 'Peças', 'href' => route('parts.index')],
            ['label' => $part->part_code, 'href' => route('parts.show', $part)],
            ['label' => 'Editar'],
        ]" />
    </x-slot:breadcrumbs>
    ...
    {{-- Header --}}
    <x-slot:header>
        Editar dados da peça {{ $part->part_code }}
    </x-slot:header>

    {{-- Header Actions --}}
    <x-slot:headerActions>
        <x-spire::button href="{{ route('parts.show', $part) }}" variant="ghost">
    ...
    <form action="{{ route('parts.update', $part) }}" method="POST" id="part-form">
        @csrf
        @method('PUT')
                ...
                <x-spire::card>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <x-spire::input type="text" name="part_code" label="Código da Peça" placeholder="Ex: PRT-001"
                            :value="old('part_code', $part->part_code)" :error="$errors->first('part_code')" required />

                        <x-spire::select name="unit" label="Unidade" :value="old('unit', $part->unit)" :options="[
                        ...
                        <div class="md:col-span-2">
                            <x-spire::input type="text" name="description" label="Descrição"
                                placeholder="Ex: Compressor 1/4HP 110V" :value="old('description', $part->description)" :error="$errors->first('description')" required />
                        </div>

                        <div class="md:col-span-2">
                            <x-spire::input type="text" name="short_description" label="Descrição Curta"
                                placeholder="Ex: Compressor 1/4HP" :value="old('short_description', $part->short_description)" :error="$errors->first('short_description')" />
                        </div>
                    </div>
                </x-spire::card>
                ...
                <x-spire::card>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <x-spire::input type="text" name="ean" label="EAN" placeholder="7890000000123"
                            :value="old('ean', $part->ean)" :error="$errors->first('ean')" maxlength="20" />

                        <x-spire::input type="text" name="ean_packaging" label="EAN Embalagem"
                            placeholder="7890000000456" :value="old('ean_packaging', $part->ean_packaging)" :error="$errors->first('ean_packaging')" maxlength="20" />

                        <x-spire::input type="text" name="manufacturer_code" label="Código do Fabricante"
                            placeholder="ABC-12345" :value="old('manufacturer_code', $part->manufacturer_code)" :error="$errors->first('manufacturer_code')" />

                        <x-spire::input type="text" name="location" label="Localização no Estoque"
                            placeholder="Ex: A1-P2-G3" :value="old('location', $part->location)" :error="$errors->first('location')" />
                    </div>
                </x-spire::card>
                ...
                <x-spire::card>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <x-spire::input type="text" name="ncm" label="NCM" placeholder="8418.69.99"
                            :value="old('ncm', $part->ncm)" :error="$errors->first('ncm')" maxlength="15" />

                        <x-spire::input type="text" name="cest" label="CEST" placeholder="21.058.00"
                            :value="old('cest', $part->cest)" :error="$errors->first('cest')" maxlength="15" />

                        <x-spire::select name="origin" label="Origem" placeholder="Selecione" :value="old('origin', $part->origin)"
                            :options="[
                ...
                <x-spire::card>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <x-spire::input type="number" name="cost_price" label="Preço de Custo" placeholder="0.00"
                            :value="old('cost_price', $part->cost_price)" :error="$errors->first('cost_price')" step="0.01" min="0" />

                        <x-spire::input type="number" name="price" label="Preço de Venda" placeholder="0.00"
                            :value="old('price', $part->price)" :error="$errors->first('price')" step="0.01" min="0" />
                    </div>
                </x-spire::card>
                ...
                <x-spire::card>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <x-spire::input type="number" name="min_stock" label="Estoque Mínimo" placeholder="0"
                            :value="old('min_stock', $part->min_stock)" :error="$errors->first('min_stock')" min="0" />

                        <x-spire::input type="number" name="max_stock" label="Estoque Máximo" placeholder="0"
                            :value="old('max_stock', $part->max_stock)" :error="$errors->first('max_stock')" min="0" />
                    </div>
                </x-spire::card>
                ...
                <x-spire::card>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <x-spire::input type="number" name="net_weight" label="Peso Líquido (kg)" placeholder="0.000"
                            :value="old('net_weight', $part->net_weight)" :error="$errors->first('net_weight')" step="0.001" min="0" />

                        <x-spire::input type="number" name="gross_weight" label="Peso Bruto (kg)"
                            placeholder="0.000" :value="old('gross_weight', $part->gross_weight)" :error="$errors->first('gross_weight')" step="0.001"
                            min="0" />

                        <div></div>

                        <x-spire::input type="number" name="width" label="Largura (cm)" placeholder="0.00"
                            :value="old('width', $part->width)" :error="$errors->first('width')" step="0.01" min="0" />

                        <x-spire::input type="number" name="height" label="Altura (cm)" placeholder="0.00"
                            :value="old('height', $part->height)" :error="$errors->first('height')" step="0.01" min="0" />

                        <x-spire::input type="number" name="depth" label="Profundidade (cm)" placeholder="0.00"
                            :value="old('depth', $part->depth)" :error="$errors->first('depth')" step="0.01" min="0" />
                    </div>
                </x-spire::card>
            </div>
                ...
                <x-spire::card>
                    ...
                    <div class="space-y-4">
                        <x-spire::checkbox name="is_active" label="Ativa" size="lg"
                            hint="Peças inativas não aparecem para seleção" :checked="old('is_active', $part->is_active)" />

                        <x-spire::checkbox name="is_display" label="É Display" size="lg"
                            hint="Marque se esta peça é para exposição" :checked="old('is_display', $part->is_display)" />
                    </div>
                </x-spire::card>
