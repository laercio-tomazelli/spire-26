{{-- Partial para requisições AJAX da tabela de Permissions --}}
@php
...
<div class="overflow-x-auto">
    <table class="{{ $classes }}" data-striped="false" data-hoverable="true">
        ...
        <x-ui.table.columns :selectable="true">
            ...
            <x-ui.table.column label="Grupo" sortable sortField="group" data-column="group" />
        ...
        <x-ui.table.body>
            @forelse ($permissions as $permission)
                <x-ui.table.row :record="$permission" :selectable="true" :clickable="true">
                    ...
                    {{-- Group --}}
                    <x-ui.table.cell data-column="group">
                        @if ($permission->group)
                            <span
                                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300 capitalize">
                                {{ $permission->group }}
                            </span>
                        @else
