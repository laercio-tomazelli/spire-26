...
class UserStoreRequest extends FormRequest
{
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator): void {
            ...
            $this->validateActorScope($validator, $authUser, $type);
    ...
    private function validateActorScope(Validator $validator, User $authUser, UserType $type): void
    {
        if ($authUser->isSpire()) {
            return;
        }
        ...
        if ($authUser->isPartnerAdmin()) {
            if ($type !== UserType::Partner) {
                $validator->errors()->add('user_type', 'Administradores de posto só podem criar usuários do próprio posto.');
            }
            ...
            if ($this->boolean('is_partner_admin')) {
                $validator->errors()->add('is_partner_admin', 'Administradores de posto não podem criar outros administradores.');
            }

            return;
        }

        if ($authUser->isSpireClient()) {
            if ($type === UserType::Spire) {
                $validator->errors()->add('user_type', 'Clientes Spire não podem criar usuários internos Spire.');
            }
        ...
        if ($authUser->isManufacturer()) {
            if ($type !== UserType::Manufacturer) {
                $validator->errors()->add('user_type', 'Fabricantes só podem criar usuários do próprio fabricante.');
            }

            if ((int) $this->input('manufacturer_id') !== $authUser->manufacturer_id) {
                $validator->errors()->add('manufacturer_id', 'O fabricante informado não pertence ao seu escopo.');
            }
